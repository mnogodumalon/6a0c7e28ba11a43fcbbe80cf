...
import type { Speisekarte, Tischverwaltung, Bestellerfassung, CreateSpeisekarte, CreateTischverwaltung, CreateBestellerfassung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- TISCHVERWALTUNG ---
  static async getTischverwaltung(): Promise<Tischverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TISCHVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Tischverwaltung[];
    return enrichLookupFields(records, 'tischverwaltung');
  }
  static async getTischverwaltungEntry(id: string): Promise<Tischverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TISCHVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Tischverwaltung;
    return enrichLookupFields([record], 'tischverwaltung')[0];
  }
  static async createTischverwaltungEntry(fields: CreateTischverwaltung) {
    return callApi('POST', `/apps/${APP_IDS.TISCHVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'tischverwaltung') });
  }
  static async updateTischverwaltungEntry(id: string, fields: Partial<CreateTischverwaltung>) {
    return callApi('PATCH', `/apps/${APP_IDS.TISCHVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'tischverwaltung') });
  }
  static async deleteTischverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TISCHVERWALTUNG}/records/${id}`);
  }

  // --- BESTELLERFASSUNG ---
  static async getBestellerfassung(): Promise<Bestellerfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BESTELLERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Bestellerfassung[];
    return enrichLookupFields(records, 'bestellerfassung');
  }
  static async getBestellerfassungEntry(id: string): Promise<Bestellerfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BESTELLERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Bestellerfassung;
    return enrichLookupFields([record], 'bestellerfassung')[0];
  }
  static async createBestellerfassungEntry(fields: CreateBestellerfassung) {
    return callApi('POST', `/apps/${APP_IDS.BESTELLERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'bestellerfassung') });
  }
  static async updateBestellerfassungEntry(id: string, fields: Partial<CreateBestellerfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.BESTELLERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bestellerfassung') });
  }
  static async deleteBestellerfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BESTELLERFASSUNG}/records/${id}`);
  }
