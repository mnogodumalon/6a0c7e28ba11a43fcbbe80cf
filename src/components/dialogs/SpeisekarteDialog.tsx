...
export function SpeisekarteDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: SpeisekarteDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "artikel_name": string | null, // Name des Artikels\n  "kategorie": LookupValue | null, // Kategorie (select one key: "vorspeise" | "hauptgericht" | "dessert" | "getraenk" | "beilage" | "sonstiges") mapping: vorspeise=Vorspeise, hauptgericht=Hauptgericht, dessert=Dessert, getraenk=Getränk, beilage=Beilage, sonstiges=Sonstiges\n  "beschreibung": string | null, // Beschreibung\n  "preis": number | null, // Preis (€)\n  "verfuegbar": boolean | null, // Verfügbar\n}`;
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="artikel_name">Name des Artikels</Label>
            <Input
              id="artikel_name"
              value={fields.artikel_name ?? ''}
              onChange={e => setFields(f => ({ ...f, artikel_name: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
