...
export function BulkEditDialog({ open, onClose, onApply, fields, selectedCount, loading, lookupLists }: BulkEditDialogProps) {
  ...
  function handleFieldChange(key: string) {
    ...
    else if (f?.type.includes('multipleapplookup') || f?.type.includes('multiplelookup')) setValue([] as string[]);
  ...
  async function handleApply() {
    ...
    if (currentField?.type.includes('multipleapplookup') && currentField.targetAppId) {
      const ids = Array.isArray(value) ? (value as string[]) : [];
      finalValue = ids.length ? ids.map(id => createRecordUrl((APP_IDS as any)[currentField.targetAppId!], id)) : null;
    } else if (currentField?.type.includes('multiplelookup')) {
      const keys = Array.isArray(value) ? (value as string[]) : [];
      finalValue = keys.length ? keys : null;
    } else if (currentField?.type.includes('applookup') && currentField.targetAppId) {
        ...
        <div className="space-y-4 py-2">
          ...
          {currentField && (
            <div className="space-y-2">
              ...
              ) : currentField.type.includes('multipleapplookup') && currentField.targetEntity ? (
                <MultiCheckboxField
                  id={currentField.key}
                  options={(lookupLists?.[currentField.targetEntity + 'List'] ?? []).map((r: any) => ({
                    id: r.record_id,
                    label: String(r.fields?.[currentField.displayField!] ?? r.record_id),
                  }))}
                  value={Array.isArray(value) ? (value as string[]) : []}
                  onChange={next => setValue(next)}
                  searchPlaceholder="Suchen..."
                />
              ) : currentField.type.includes('multiplelookup') && currentField.options ? (
                <MultiCheckboxField
                  id={currentField.key}
                  options={currentField.options.map(o => ({ id: o.key, label: o.label }))}
                  value={Array.isArray(value) ? (value as string[]) : []}
                  onChange={next => setValue(next)}
                  searchPlaceholder="Suchen..."
                />
